...
public class Camera2BasicFragment extends Fragment
     ...
     * 카메라 전면, 후면 아이디 값
     **/
    private int facingid = 0; // 전면화면이 0이므로 0.

    /**
     * 자동 포커스 체크
     **/
    private boolean mAutoFocusSupported; // 전면화면이 0이므로 0.
    ...
    private void openCamera(int width, int height) {
        ...
        setUpCameraOutputs(width, height, facingid); // facingid의 초기값은 CameraCharacteristics.LENS_FACING_FRONT
        configureTransform(width, height);
        Activity activity = getActivity();
        CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        try {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.picture:
                ...
                break;
            case R.id.change:
                if(facingid == CameraCharacteristics.LENS_FACING_FRONT) { // 전면카메라일 때 후면카메라로, 후면카메라 일 때 전면카메라로 변경해줌.
     ...
     * Saves a JPEG {@link Image} into the specified {@link File}.
     */
    private static class ImageUpLoder implements Runnable {
